...
@section('css')
<!-- select 2 -->
<link href="{{asset('public/admin/assets/css/select_2.css')}}" rel="stylesheet">
@endsection
@section('content')
                    ...
                    <div class="row">
                        ...
                        <div class="col-xs-3">
                            <div id="dataTables-example_filter" class="dataTables_filter">
                                <select class="form-control" name="country_name" id="countries"
                        ...
                        <div class="col-xs-3">
                            <div id="dataTables-example_filter" class="dataTables_filter">
                                <input name="date_from" type="date" class="form-control input-sm"
                                    aria-controls="dataTables-example">
                            </div>
                        </div>
                        <div class="col-xs-3">
                            <div id="dataTables-example_filter" class="dataTables_filter">
                                <input name="date_to" type="date" class="form-control input-sm"
                                    aria-controls="dataTables-example">
                            </div>
                        </div>
                        <div class="col-xs-12" style="margin:10px">
                            <input style="margin:10px" name="peak_time" type="checkbox" value="1">PeakTime
                            <input style="margin:10px" name="countries" type="checkbox" value="1">Countries
                            <input style="margin:10px" name="pages" type="checkbox" value="1">Pages
                            <input style="margin:10px" name="setions_items_rep" type="checkbox" value="1">Section Items
                            <button style="margin:10px" type="submit" class="btn btn-success btn-sm">Submit</button>
                            <a style="margin:10px" href="{{route('admin/views/index')}}"
...
@section('script')
<!-- select 2 -->
<script src="{{asset('public/admin/assets/js/select_2.js')}}"></script>
<script>
$('#countries').select2({
    ajax: {
        ...
        success: function(data) {
            if (data.length > 0) {
                ...
                for (let i = 0; i < data.length; i++) {
                    ...
                    if (data[i].section_no == 1) {
                        section_no = 'products'
                    } else if (data[i].section_no == 2) {
                        section_no = 'articles'
                    } else if (data[i].section_no == 3) {
                        section_no = 'webinars'
                    } else if (data[i].section_no == 4) {
                        section_no = 'services'
                    } else if (data[i].section_no == 5) {
                        section_no = 'trainings'
                    }

                    if (data[i].hour == 1) {
                        hour = 'صباحا بعد منتصف الليل'
                    } else if (data[i].hour == 2) {
                        hour = 'صباحا قبل الظهر'
                    } else if (data[i].hour == 3) {
                        hour = 'مساء بعد الظهر'
                    } else if (data[i].hour == 4) {
                        hour = 'مساء بعد الخامسه ليلا'
                    }

                    if (typeof(data[i].viewable_name) == 'undefined') {} else {
